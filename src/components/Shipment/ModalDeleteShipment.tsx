...
import { onErrorMessageShipment } from "../../store";
import { Alert } from "..";
...
const ModalDeleteShipment = ({
  ...
  const { startDeleteShipment } = useShipment();
  ...
  const handleClose = () => {
    ...
    dispatch(onErrorMessageShipment(null));
    setModalDelete(!modalDelete);
  };

  const handleSubmit = async () => {
    await startDeleteShipment(shipment!);
  };
  ...
  return (
    <Transition.Root show={modalDelete} as={Fragment}>
      <Dialog
        as="div"
        className="fixed z-10 inset-0 overflow-y-auto -mt-64 md:-mt-0"
        onClose={handleClose}
      >
                ...
                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                  ...
                  <div className="mt-2">
                    <p className="text-sm text-gray-500">
                      A deleted shipment cannot be restored
                    </p>
                  </div>
                </div>
              </div>
